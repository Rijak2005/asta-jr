module.exports = {
  ...
  execute: async (message, args, bot) => {
    ...
    message.reply(
      "So i heard you want to be premium? huh.. Let's see if you are worth it! To embark on this quest ***you will need 40k DMC*** and ***a screenshot of your subscription to professor astatine!*** you have 2 minutes to collect these and then i will start the quest. So be quick!"
    );
    ...
    setTimeout(async () => {
      ...
      await message.channel.awaitMessages(filter, {max:2, time: 33000 }).then(async (collected) => {
        for (var mes of collected.toJSON()) {
            ...
            if (mes.content.includes("You gave Mr.Astatine **⏣ 32609**") && mes.authorID === "270904126974590976"&&mes.content.includes(message.author.id)) {
                transaction = true;
                break;
            }
            transaction = false;
        ...
        //  message.reply(transaction===true ? "Looks like you have the money noice...": "You poor son of your mom you didn't give money why would you want to waste my time!?");
          ...
          message.reply("What a noob you couldn't complete this in 30 seconds OMG!");
    ...
    if (transaction) {
        ...
        message.reply("You poor son of your mom you didn't give money why would you want to waste my time!?");
        message.channel.bulkDelete(4);
        return;
    }
        message.reply(
      ...
      message.channel.awaitMessages(filter, {max:1, time: 58000 }).then((collectedMes) => {
        if (!collectedMes.first().attachments.first()) {
            ...
            bot.users.cache
              ...
              ch.send(`<@${message.author.id}>`,{
                embed: {
                  ...
                  description: "You have completed the **Quest of the Premium 💎**, proving that you are absolutely worthy to become a premium member!\n I bow to you 🙇‍♂️\nAnd now you'll be rewarded with the premium role!\n ENJOYY!",
                  color: 0x2ac7b1,
                },
              });
            });
          ...
          message.reply("What a noob you couldn't even send your ss in 55 seconds OMG! Now your money is Lost! lmao");
          console.log(err);
        });
    }, 120000);    
  },
